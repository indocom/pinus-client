...
function getRecipient(recipientName: string) {
  ...
  .then(entries => entries.items.filter(entry => entry.fields.name[LOCALE] === recipientName))
  .then(entries => {
    if (entries.length !== 1) {
      throw new Error(`There exists more than one match to ${recipientName} which is impossible. Aborting..`);
    }
    return entries[0];
  });
}
...
export async function createAndLink(writerName: string, recipientName: string, content: string) {
  let [contentEntry, recipientEntry] = await Promise.all([createContent(content, writerName), getRecipient(recipientName)]);
